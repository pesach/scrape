...
async def fetch_youtube_url_by_id(youtube_url_id: str) -> str | None:
    if not SUPABASE_URL or not SUPABASE_KEY:
        logger.error("SUPABASE_URL or SUPABASE_KEY not set in .env; cannot fetch youtube_url by id.")
        return None

    endpoint = SUPABASE_URL.rstrip("/") + "/rest/v1/youtube_urls"
...
async def process_job(job):
    ...
    try:
        ...
        if not url:
            try:
                job_map = job.dict() if hasattr(job, "dict") else (
                    job.model_dump() if hasattr(job, "model_dump") else dict(job.__dict__)
                )
                for key in ("url", "link", "source", "video_url", "youtube_url"):
                    if key in job_map and job_map[key]:
                        url = job_map[key]
                        break
                if not url and ("youtube_url_id" in job_map and job_map["youtube_url_id"]):
        ...
        # 🔥 Override: force VIDEO if single video URL
        if url and "watch?v=" in url and url_type_enum != URLType.VIDEO:
            logger.warning(f"[{job_id}] Overriding url_type={url_type_enum} → VIDEO (single video URL detected)")
